...
class Reconnecting_VideoGear:
    def __init__(self, cam_address, stabilize=False, reset_attempts=50, reset_delay=5, back_end=cv2.CAP_FFMPEG):
        ...
        self.source = VideoGear(
            source=self.cam_address, stabilize=self.stabilize,backend=self.back_end
        ).start()
        self.running = True
